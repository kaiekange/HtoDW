...
if [ ! -f "${logfile}" ] || [ ! -f "${outfile}" ] || ! grep -q "Begin processing the 1000th record" "${logfile}" || ! grep -q "dropped waiting message count 0" "${logfile}"; then
    ...
    if grep -q "Begin processing the 1000th record" "${logfile}" && grep -q "dropped waiting message count 0" ${logfile}; then
