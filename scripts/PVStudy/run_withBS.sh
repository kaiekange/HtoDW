...
if [ ! -f "${logfile}" ] || [ ! -f "${outfile}" ] || ! grep -q "Begin processing the 200th record" "${logfile}" || ! grep -q "dropped waiting message count 0" "${logfile}"; then
    ...
    echo "Tuple ${process} not produced, now producing tuple" >> job_withBS.out
    ...
    if grep -q "Begin processing the 200th record" "${logfile}" && grep -q "dropped waiting message count 0" ${logfile}; then
        echo "Tuple ${process} successfully produced" >> job_withBS.out
    else
        echo "ERROR: Tuple ${process} production failed" >> job_withBS.out
    fi

else
    echo "Tuple ${process} already successfully produced" >> job_withBS.out
fi
