...
if [ ! -f "${logfile}" ] || [ ! -f "${outfile}" ] || ! grep -q "dropped waiting message count 0" "${logfile}"; then
    ...
    echo "Tuple ${process} not produced, now producing tuple" >> ${mode}.out
    ...
    if grep -q "dropped waiting message count 0" ${logfile}; then
        echo "Tuple ${process} successfully produced" >> ${mode}.out
    else
        echo "ERROR: Tuple ${process} production failed" >> ${mode}.out
    fi

else
    echo "Tuple ${process} already successfully produced" >> ${mode}.out
fi
