...
if [ ! -f "${logfile}" ] || [ ! -f "${outfile}" ] || ! grep -q "Begin processing the 200th record" "${logfile}" || ! grep -q "dropped waiting message count 0" "${logfile}"; then
    ...
    cmssw-el7 -- "cd ${HOME}Analysis/CMSSW_10_6_30_patch1/src/; cmsenv; ls EDAnalyzers/GenParticleAnalyzer/test/; cmsRun ${cfgfile} file:${infile} file:${outfile}" &> ${logfile}
